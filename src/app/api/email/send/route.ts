...
export async function POST(request: Request) {
  ...
  const rate = rateLimit(`email-send:${user.id}`, 30, 60_000);
  if (!rate.allowed)
  ...
  const risk = assessMessageRisk(`${input.subject}\n${input.body}`);
  if (risk.level === "block")
    return NextResponse.json(
      { error: "This message was blocked by the send-risk check.", risk },
      { status: 422 },
    );
  const unique = new Set<string>();
  const dedupe = (values: string[]) =>
    values.filter((value) => !unique.has(value) && unique.add(value));
  ...
  try {
    ...
    const thread = await db.mailThread.upsert({
      where: {
        mailboxId_providerId: {
          ...
          providerId: sent.threadId,
        },
      },
      update: { subject: input.subject, lastMessageAt: occurredAt },
      create: {
        mailboxId: mailbox.id,
        providerId: sent.threadId,
        subject: input.subject,
    ...
    const message = await db.mailMessage.upsert({
      where: {
        mailboxId_providerId: { mailboxId: mailbox.id, providerId: sent.id },
      },
      update: {},
      create: {
        mailboxId: mailbox.id,
        ...
        bodyEncrypted: encryptPrivateContext(
          { body: input.body },
          env().PRIVATE_CONTEXT_ENCRYPTION_KEY,
        ),
        bodyFingerprint: fingerprint(input.body, user.id),
        occurredAt,
      },
    });
    const participants = [
      { email: mailbox.email, role: "from" },
      ...to.map((value) => ({ email: value, role: "to" })),
      ...cc.map((value) => ({ email: value, role: "cc" })),
      ...bcc.map((value) => ({ email: value, role: "bcc" })),
    ];
    ...
    for (const recipient of [...to, ...cc, ...bcc])
      await db.contact.upsert({
        where: {
          userId_primaryEmail: { userId: user.id, primaryEmail: recipient },
        },
        update: {
