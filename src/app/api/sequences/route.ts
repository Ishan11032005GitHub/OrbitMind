...
const createSchema = z
  ...
  .superRefine((value, context) => {
    if (value.threadMode === "existing" && !value.threadId)
      context.addIssue({
        code: "custom",
        path: ["threadId"],
        message: "Choose or enter a thread.",
      });
    const times = value.steps.map((step) => Date.parse(step.scheduledAt));
    if (times.some((time) => !Number.isFinite(time) || time <= Date.now()))
      context.addIssue({
        code: "custom",
        path: ["steps"],
        message: "Every scheduled time must be in the future.",
      });
    if (times.some((time, index) => index > 0 && time <= times[index - 1]))
      context.addIssue({
        code: "custom",
        path: ["steps"],
        message: "Sequence steps must be scheduled in chronological order.",
      });
  });
...
export async function GET() {
  ...
  const sequences = await db.sequence.findMany({
    ...
    include: {
      ...
      enrollments: { include: { deliveries: true } },
    },
  });
  return NextResponse.json({
    sequences: sequences.map((sequence) => {
      const deliveries = sequence.enrollments.flatMap(
...
export async function POST(request: Request) {
  ...
  if (!user)
    return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  const rate = rateLimit(`sequence-create:${user.id}`, 15, 60_000);
  if (!rate.allowed)
    return NextResponse.json(
      { error: "Too many sequence requests." },
  ...
  if (!parsed.success)
    return NextResponse.json(
      { error: parsed.error.issues[0]?.message ?? "Invalid sequence" },
      { status: 400 },
    );

  const input = parsed.data;
  const risks = input.steps.map((step, index) => ({
    step: index + 1,
    ...assessMessageRisk(`${step.subject}\n${step.body}`),
  }));
  if (risks.some((risk) => risk.level === "block"))
    return NextResponse.json(
      { error: "A sequence step was blocked by the send-risk check.", risks },
      { status: 422 },
    );
  const sendingEnabled =
    env().ENABLE_SEQUENCE_SENDING === "true" &&
    user.email !== "demo@stealth.local";
  const scheduled = input.steps.map((step) => new Date(step.scheduledAt));
  const result = await db.$transaction(async (tx) => {
    const contacts = await Promise.all(
      input.to.map((recipientEmail) =>
        tx.contact.upsert({
          where: {
            userId_primaryEmail: {
              userId: user.id,
          ...
          create: input.steps.map((step, index) => ({
            ...
            delayMinutes:
              index === 0
                ? 0
                : Math.round(
                    (scheduled[index].getTime() -
                      scheduled[index - 1].getTime()) /
                      60_000,
                  ),
    ...
    for (const contact of contacts) {
      const enrollment = await tx.enrollment.create({
        data: {
          ...
          status: sendingEnabled ? "ACTIVE" : "PAUSED",
          nextRunAt: scheduled[0],
        },
      });
      ...
      await tx.intelligenceRecord.create({
        data: {
          userId: user.id,
          contactId: contact.id,
          kind: "sequence-addressing",
          version: "1",
          evidence: {
            sequenceId: sequence.id,
